...
// 履歴ありの教師局面を作成する。100万局面で134MB。
void make_teacher_with_history(std::istringstream& ssCmd) {
    std::string recordFileName;
    std::string outputFileName;
    int threadNum;
    s64 teacherNodes; // 教師局面数
    ...
    auto func = [&omutex, &ofs, &imutex, &ifs, &inputFileDist, &teacherNodes](Position& pos, std::atomic<s64>& idx, const int threadID) {
        ...
        while (idx < teacherNodes) {
            ...
            std::unordered_set<Key> keyHash;
            StateListPtr states = StateListPtr(new std::deque<StateInfo>(1));
            std::vector<HuffmanCodedPosWithHistoryAndEval> hcphevec;
            std::deque<HuffmanCodedPos> historyPos;
            std::deque<u16> historyMove16;
            GameResult gameResult = Draw;
            int cnt = 0;
            for (Ply ply = pos.gamePly(); ply < 256; ++ply, ++cnt) { // 256 手くらいで終了しておく。
                const Key key = pos.getKey();
                ...
                // 履歴保存
                historyPos.emplace_back(pos.toHuffmanCodedPos());
                historyMove16.emplace_back(static_cast<u16>(bestMove.value()));

                // 履歴7手未満の場合、教師局面を生成しない
                if (cnt >= 7)
                {
                    hcphevec.emplace_back(HuffmanCodedPosWithHistoryAndEval());
                    HuffmanCodedPosWithHistoryAndEval& hcphe = hcphevec.back();
                    hcphe.hcp = historyPos.front();
                    for (size_t i = 0; i < 7; i++) {
                        hcphe.historyMove16[i] = historyMove16[i];
                    }
                    historyPos.pop_front();
                    historyMove16.pop_front();
                    auto& pv = pos.searcher()->threads.main()->rootMoves[0].pv;
                    const Color rootTurn = pos.turn();
                    StateInfo state[MaxPly+7];
                    StateInfo* st = state;
                    ...
                    const Score eval = evaluate(pos, ss+1);
                    // root の手番から見た評価値に直す。
                    hcphe.eval = (rootTurn == pos.turn() ? eval : -eval);
                    hcphe.bestMove16 = static_cast<u16>(pv[0].value());
            ...
            if (gameResult != Draw) {
                idx += hcphevec.size();
                // 勝敗を1局全てに付ける。
                for (auto& elem : hcphevec)
                    elem.gameResult = gameResult;
                std::unique_lock<Mutex> lock(omutex);
                ofs.write(reinterpret_cast<char*>(hcphevec.data()), sizeof(HuffmanCodedPosWithHistoryAndEval) * hcphevec.size());
            }
        }
    };
    auto progressFunc = [&teacherNodes] (std::atomic<s64>& index, Timer& t) {
...
void Searcher::doUSICommandLoop(int argc, char* argv[]) {
    ...
    do {
        ...
        else if (token == "make_teacher_with_history") {
            if (!evalTableIsRead) {
                std::unique_ptr<Evaluator>(new Evaluator)->init(options["Eval_Dir"], true);
                evalTableIsRead = true;
            }
            make_teacher_with_history(ssCmd);
        }
